...
# ========= 依存のプレチェック =========
def _check_predict_deps() -> list[str]:
    missing = []
    try:
        import joblib  # noqa
    except Exception:
    ...
    # XGBoost モデルを使っている場合のみ必要
    try:
        import xgboost  # noqa
    except Exception:
        # 使ってなければスキップ。必要なら requirements に追加。
        pass
    return missing
...
def run(cmd: list[str], cwd: Path = ROOT) -> tuple[int, str]:
    try:
        p = subprocess.run(cmd, cwd=str(cwd), text=True, capture_output=True, shell=False, env=_env_with_src())
...
def app_main():
    ...
    # 依存のプレチェック
    miss = _check_predict_deps()
    if miss:
        st.error("予測に必要な依存が不足しています: " + ", ".join(miss) +
                 "\n→ requirements.txt に追加して再デプロイしてください。")
        # 進めても落ちるのでここで停止
        st.stop()
    ...
    # ========= EVレポート =========
    df_ev = read_csv_safe(EV_CSV) or pd.DataFrame()
    if not df_ev.empty:
        ...
        if date_col is None:
            ...
            if "実績" in payout_mode:
                ...
                if paymap is not None and not paymap.empty:
                    ...
                    payout_series = payout_series.where((payout_series >= 10000) & (payout_series <= 300000), np.nan).fillna(float(payout))
                else:
            ...
            if "joint_prob" in df_win.columns:
                ...
                chart = (ideal_line + points + labels).properties(width="container", height=360, title="予測確率の信頼度カーブ（y=x が理想）")
...
try:
    ...
    st.title("Numbers3 Dashboard – Panic Mode")
    st.error("起動時に例外が発生しました。トレースを表示します。")
